# -*- coding: utf-8 -*-
"""
03_eda_synthetic.py — Fase 3: EDA pada Data Sintetis
======================================================
TUJUAN:
  Membuktikan secara visual bahwa behavioral shift BENAR-BENAR ada di data.
  Ini bukan sekadar distribusi — kita harus tunjukkan KAPAN dan BAGAIMANA
  pola berubah. Inilah yang membedakan EDA kita dari tim lain.

OUTPUT:
  - 6 chart bukti behavioral shift
  - Timeline akun escalating (slide utama presentasi)
  - Heatmap korelasi fitur
  - data/eda_summary.csv (ringkasan per profil)
"""
...
# ── Konfigurasi visual ──────────────────────────────────────────────────────
# Warna per profil — konsisten di semua chart
PALETTE = {
    'normal'       : '#3498db',
    'early_stage'  : '#f1c40f',
    'escalating'   : '#e67e22',
...
# ════════════════════════════════════════════════════════════════════════════
"""
Langkah pertama: pastikan data yang masuk sudah benar sebelum analisis.
Kita tidak mau membangun kesimpulan di atas data yang kotor.
"""
...
# ════════════════════════════════════════════════════════════════════════════
"""
INI ADALAH BAGIAN PALING PENTING DARI EDA.

Kita tidak hanya menunjukkan distribusi data — kita MEMBUKTIKAN bahwa
ada pola yang berbeda antara profil normal dan berisiko.

Setiap chart menjawab satu pertanyaan spesifik:
1. Jam berapa mereka aktif?
2. Seberapa sering malam hari?
3. Seberapa sering bertransaksi?
4. Ke berapa banyak orang mereka kirim uang?
5. Seberapa banyak pakai QRIS?
6. Apakah ada pergeseran ke malam? (TEMPORAL SHIFT — fitur kunci)
"""
...
# ── Chart 1: Distribusi jam aktivitas ──────────────────────────────────────
# Pertanyaan: apakah profil berisiko lebih aktif di malam hari?
...
# Insight yang diharapkan: escalating & heavy_gambler tinggi di jam 22–04

# ── Chart 2: Night ratio per profil ────────────────────────────────────────
# Pertanyaan: secara rata-rata, seberapa besar porsi aktivitas malam?
...
# ── Chart 3: Frekuensi transaksi per 24 jam ────────────────────────────────
# Pertanyaan: apakah profil berisiko bertransaksi lebih sering?
# PPATK: bandar menginstruksikan pemain memecah transaksi → frekuensi naik
...
# ── Chart 4: Penerima unik per 7 hari ──────────────────────────────────────
# Pertanyaan: ke berapa banyak akun berbeda mereka kirim uang?
# Smurfing: uang dipecah ke banyak akun penerima untuk sembunyikan jejak
...
# ── Chart 5: QRIS ratio ─────────────────────────────────────────────────────
# Pertanyaan: apakah profil berisiko beralih ke QRIS?
# PPATK 2025: pergeseran ke QRIS untuk percepat perpindahan dana
...
# ── Chart 6: TEMPORAL SHIFT SCORE — FITUR KUNCI ────────────────────────────
# Pertanyaan: apakah ada pergeseran ke malam dibanding minggu sebelumnya?
# Nilai positif = akun ini BERGERAK ke aktivitas malam → sinyal awal eskalasi
# Ini adalah fitur yang paling unik dan membedakan sistem kita
...
plt.show()
print("\n Chart tersimpan: data/eda_behavioral_shift.png")
...
# ════════════════════════════════════════════════════════════════════════════
"""
INI ADALAH CHART PALING PENTING UNTUK PRESENTASI.

Kita ambil satu akun escalating dan tunjukkan perubahan perilakunya
dari waktu ke waktu. Juri non-teknis harus bisa melihat dengan mata:
"oh, polanya memang berubah sebelum jadi berisiko."

Ini yang tidak bisa dilakukan oleh sistem konvensional — mereka tidak
punya fitur temporal shift, jadi tidak bisa melihat perubahan ini.
"""
...
# Tandai zona perubahan (sepertiga akhir dari data)
...
# ════════════════════════════════════════════════════════════════════════════
"""
Tujuan: identifikasi fitur mana yang paling berkorelasi dengan is_at_risk.
Ini digunakan untuk:
1. Validasi bahwa feature engineering kita memang relevan
2. Menentukan fitur mana yang akan masuk ke model
3. Menjawab pertanyaan juri: "kenapa pilih fitur ini?"
"""
...
corr_with_label = corr_matrix['is_at_risk'].drop('is_at_risk').sort_values(key=abs, ascending=False)
...
# ════════════════════════════════════════════════════════════════════════════
"""
Membuat tabel ringkasan yang bisa dimasukkan ke slide presentasi.
Tabel ini menjawab pertanyaan juri: "seberapa berbeda antar profil?"
"""
